...
검색되지 않아도 / 지워지는 튜플이 없어도 / 수정되는 튜플이 없어도 에러가 발생하지 않는다.(그냥 검색, 변화가 없을 뿐)

controller의 try-catch 구문에서

검색/변화가 없어도 try문이 정상적으로 실행된다.

###### INSERT

기본키로 이미 값이 있는 속성에 중복으로 INSERT하려고 하면 키가 중복된다는 에러가 발생한다.

Exception이 발생해 catch문을 실행한다.

---
##### test 안에 들어가는 변수

<if test="reviewIndex == 0"></if>
<when test="reviewIndex == 0"></when>

test에 들어가는 변수는 ${}, #{}로 묶지 않고 바로 써줘야 한다. 아니면 null 값이 된다.
